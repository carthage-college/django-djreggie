...
class DataReqModel(models.Model):
    name = models.CharField(max_length=250)
    
    CHOICES_GROUPING = (
        'Faculty','Staff','Student'
    )
    grouping = models.CharField(maxlength=10, choices=CHOICES_GROUPING)
    deptorg = models.CharField(maxlength=250, null=True, blank=True, default="")
    email = models.EmailField(maxlength=250)
    phone = models.CharField(maxlength=15)
    date_requested = models.DateField(default=datetime.date.today)
    date_needed = models.DateField()
    data_description = models.TextField()
    data_use = models.TextField()
