...
class ConsentModel(models.Model):
    ...
    def save(self): #this is for putting data in the database
        selectFerpaCountSQL = '''SELECT COUNT(ferpafamily_no) AS cnt
                FROM cc_stg_ferpafamily
                WHERE student_id = %s''' % (self.__dict__['student_id'])
        entry = do_sql(selectFerpaCountSQL, key=settings.INFORMIX_DEBUG, earl=settings.INFORMIX_EARL).first()['cnt']
        #check if an entry already exists in database. if it does then don't create a new one
        if not entry:
            insertFerpaSQL = '''INSERT INTO cc_stg_ferpafamily (student_id, datecreated)
...
class ParentForm(models.Model): 
    ...
    SHARE_AREA = ( #Here we make some choices that will be assigned to a field
        ("ACADEMIC", 'Academic Records'),
        ("FINANCIAL", 'Financial Records'),
        ("BOTH", 'I would like to share both'),
        ("NEITHER", 'I would like to share neither'),
    )
    share = models.CharField(choices=SHARE_AREA, max_length=200)    #This is the field we assign them to
    name = models.CharField(max_length=100)
    phone = models.CharField(max_length=16)
    email = models.EmailField()
    RELATIVE = (    #More choices
        ("MOM", 'Mother'),
        ("DAD", 'Father'),
        ("GRAN", 'Grandparent'),
        ("BRO", 'Brother'),
    ...
    relation = models.CharField(choices=RELATIVE, max_length=20) #Stick those choices in here